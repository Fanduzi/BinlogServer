...
create_task() {
  ...
  resp="$(curl -fsS -X POST "$API/api/tasks" \
    ...
    -d "{\"name\":\"e2e-observability-${RUN_TAG}\",\"source\":{\"host\":\"127.0.0.1\",\"port\":13306,\"user\":\"repl\",\"password\":\"replpass\",\"flavor\":\"mysql\",\"server_id\":${sid}},\"start\":{\"mode\":\"LATEST\"},\"storage\":{\"retention_days\":7}}")"
  local id
