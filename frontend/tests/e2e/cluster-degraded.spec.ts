...
test('cluster degraded scenario shows offline worker and mixed-risk tasks', async ({ page }) => {
  await registerMockRoutes(page, { scenario: 'cluster-degraded' })
  await page.goto('/')

  await expect(page.getByText('2 个 Worker')).toBeVisible()
  await expect(page.locator('.cluster-worker-item', { hasText: 'worker-b' })).toBeVisible()
  await expect(page.locator('.cluster-worker-item', { hasText: 'worker-b' }).getByText('离线')).toBeVisible()
