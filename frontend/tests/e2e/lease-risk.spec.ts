...
test('lease risk scenario surfaces stale lease labels in list and detail views', async ({ page }) => {
  await registerMockRoutes(page, { scenario: 'lease-risk' })
  await page.goto('/')

  await expect(page.locator('tr', { hasText: 'task-stale-lease' }).getByText('风险', { exact: true })).toBeVisible()
  await page.getByTestId('task-detail-trigger-501').click()
  await expect(page.getByTestId('task-drawer')).toBeVisible()
  await expect(page.locator('.detail-panel', { hasText: 'Lease 与 Worker' }).getByText('Lease 状态')).toBeVisible()
  await expect(page.locator('.detail-panel', { hasText: 'Lease 与 Worker' }).getByText('worker-stale')).toBeVisible()
})
