...
| `src/composables/useDashboard.js` | Dashboard/cluster 响应状态容器，只信 dashboard 的 `total/limit/offset`，保留 starting 与 running 独立计数 |
...
- 任务观测：控制台任务页只走 `/api/dashboard` 一次读取。页、汇总、按源计数同一物化；缺少 `total/limit/offset` 时拒绝应用，不再当旧包本地切页。state/host/port 过滤随请求发送，关键词/源库文本/复制状态/告警/排序明确仅作用于当前页。
