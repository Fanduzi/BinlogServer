...
|------|---------------|
| main.js | Vue 应用入口 |
| App.vue | 运维控制台主页面，含左侧整列可折叠菜单（左下折叠控件）、多工作区分区（总览/任务/源库/Worker/告警）与 `/#/...` 深链定位、明确的全局/当前页筛选范围、详情抽屉与设置流程；刷新/翻页只走 `useDashboard.refreshAll`，列表租约风险用任务抄本 |
...
- 任务页只消费 dashboard 一次读取：`total/limit/offset`、summary 与按源计数同一物化；缺少分页字段时拒绝应用，不再当旧包本地切页。任务状态筛选为服务端全局筛选，其余任务筛选与排序明确为当前页行为。刷新/翻页只编排一次 dashboard + cluster + workers；列表租约风险用任务上的主人/epoch，`/lease` 只留给单任务详情。
